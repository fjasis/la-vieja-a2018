...
void jugadacomputadordificil(char matriz[3][3], char t, char caracterus, int *bol);

char matriz[3][3], caracterus, turno, band = 'F';
int i, j, dificultad, modo, bol = 0, *b, *m, *d;

jugador jugador1, jugador2;

int main() {

	m = &modo; d = &dificultad; b = &bol;
	srand(time(NULL));
	seleccionmodo(d, m);
	datos(modo);
		...
		if (turno == caracterus) {
			...
			if (modo == 2) {
				...
				if (dificultad == 1) {
					...
					jugadacomputadordificil(matriz, turno, caracterus, b);

...
char caracterus=seleccioncaracter();
...
int evaluacion(char b[3][3],char caracterus,char oponente){//evaluate
    int fil,col;
...
int minimax(char matriz[3][3], int depth, bool isMax)
{
    ...
    // it is a tie
    if (Siquedanmovimientos(board)==false){
        return 0;
    }
...
Move buscamejormovida(char matriz[3][3]){//findBestove
    ...
    for (i = 0; i<3; i++){
        for (int j = 0; j<3; j++){
            ...
            if (matriz[i][j]==' '){
                ...
                // move.
                int movidaVal = minimax(board, 0, false);
